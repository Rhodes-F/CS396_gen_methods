...
class BasicSystem extends ParticleSystem {
  ...
  static desc = "dancers"; // Ignore the Glitch parse error
...
class BasicParticle extends Particle {
  constructor(ps, index) {
    ...
    let r = 10 + index*2
    let theta = index*.3
    this.pos.setToPolar(150, theta/6).add(p.width/2, p.height/2)
  ...
  calculateForces(p, dt) {
   ...
   // We can attract it to the center
    ...
    this.attractionForce = this.pos.getForceTowardsPoint(mouse, 1, { falloff:.8 } )
    // this.v.mult(.99)
