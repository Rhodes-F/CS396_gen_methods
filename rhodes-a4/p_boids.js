...
class BoidSystem extends ParticleSystem {
  static label = "⚡️"; // Ignore the Glitch parse error
  static desc = "color changing ships with blur"; // Ignore the Glitch parse error
...
class BoidParticle extends Particle {
  ...
  draw(p, drawDebug = false) {
    ...
    p.noStroke();
//     let colors_nums=255;
//     for (let i = 0; i < colors_nums; i++) {
//       p.fill(colors_nums,100,50,1);
//     }
    let color = t*10 %255
    p.fill(t*10,100,50,1)
    p.push();
    p.translate(...this.pos);
    p.rotate(this.angle);

    p.beginShape();
    p.vertex(this.radius+2, 0);
    p.vertex(-this.radius, -this.radius);
    p.vertex(this.radius, -this.radius);
    p.vertex(this.radius/4, this.radius/4);
    p.vertex(0, 0);
    p.vertex(-this.radius, this.radius);
